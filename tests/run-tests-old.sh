...
# Simple YAML parser for test suite configuration
parse_yaml_config() {
    local config_file="$1"
    ...
    while IFS= read -r line; do
        ...
        if [[ "$line" =~ ^[[:space:]]*-[[:space:]]*suite: ]]; then
            in_suite=true
            current_prefix=""
            current_shortname=""
            current_description=""
        ...
        if [[ "$in_suite" == "true" ]]; then
            # Extract fields - handle both quoted and unquoted values
            if [[ "$line" =~ ^[[:space:]]*prefix:[[:space:]]*\"?([^\"]+)\"?[[:space:]]*$ ]]; then
                current_prefix="${BASH_REMATCH[1]}"
            elif [[ "$line" =~ ^[[:space:]]*shortname:[[:space:]]*\"?([^\"]+)\"?[[:space:]]*$ ]]; then
                current_shortname="${BASH_REMATCH[1]}"
            elif [[ "$line" =~ ^[[:space:]]*description:[[:space:]]*\"?([^\"]+)\"?[[:space:]]*$ ]]; then
                current_description="${BASH_REMATCH[1]}"
                
                # End of suite definition - process it
                if [[ -n "$current_prefix" && -n "$current_shortname" && -n "$current_description" ]]; then
                    # Convert prefix pattern to actual prefixes
                    if [[ "$current_prefix" =~ ^([0-9]+)\*$ ]]; then
                        # Single prefix like "1*" -> "10"
                        local base_prefix="${BASH_REMATCH[1]}"
                        local numeric_prefix="${base_prefix}0"
                        
                        SUITE_GROUPS["$numeric_prefix"]="$current_shortname"
                        SUITE_SHORTNAMES["$numeric_prefix"]="$current_shortname"
                        SUITE_DESCRIPTIONS["$numeric_prefix"]="$current_description"
                        
                        # Map shortname to prefixes
                        if [[ -n "${SHORTNAME_TO_PREFIXES[$current_shortname]:-}" ]]; then
                            SHORTNAME_TO_PREFIXES["$current_shortname"]+=" $numeric_prefix"
                        else
                            SHORTNAME_TO_PREFIXES["$current_shortname"]="$numeric_prefix"
                        fi
                    fi
