...
parse_yaml_config() {
    ...
    while IFS=$'\t' read -r prefix shortname description critical; do
        ...
        if [[ "$prefix" =~ ^([0-9]+)\*$ ]]; then
            ...
            # Set critical flag (default to false if not specified)
            if [[ "$critical" == "true" ]]; then
                SUITE_CRITICAL["$numeric_prefix"]="true"
            else
                SUITE_CRITICAL["$numeric_prefix"]="false"
            fi
    ...
    done < <(yq eval '.suites[].suite | [.prefix, .shortname, .description, .critical] | @tsv' "$config_file")
...
discover_test_scripts() {
    ...
    for script in "$TEST_DIR"/[0-9][0-9]-*.sh; do
        if [[ -f "$script" && -x "$script" ]]; then
            ...
            # Extract test timeout from script (default: 60 seconds)
            local timeout="60"
            if grep -q "^# TEST_TIMEOUT:" "$script"; then
                timeout=$(grep "^# TEST_TIMEOUT:" "$script" | sed 's/^# TEST_TIMEOUT: *//')
                # Validate timeout is a number
                if ! [[ "$timeout" =~ ^[0-9]+$ ]]; then
                    print_warning "Invalid timeout '$timeout' in $script, using default 60s"
                    timeout="60"
                fi
            fi
...
# Check if a test belongs to a critical suite
is_critical_test() {
    local test_name="$1"
    local prefix="${test_name:0:2}"
    
    if [[ -n "${SUITE_CRITICAL[$prefix]:-}" ]] && [[ "${SUITE_CRITICAL[$prefix]}" == "true" ]]; then
        return 0  # true - this is a critical test
    else
        return 1  # false - this is not a critical test
    fi
}
...
run_test_script() {
    ...
    echo "Timeout: ${timeout}s" >> "$FULL_REPORT"
    echo "Started: $(date)" >> "$FULL_REPORT"
    echo "" >> "$FULL_REPORT"
    
    if [[ "$VERBOSE_MODE" == "true" ]]; then
        print_info "Running in verbose mode - output will be shown in real-time"
        # Use timeout with tee to both display output and save to file
        if timeout "$timeout" "$script" 2>&1 | tee "$test_report"; then
        ...
        if [[ $exit_status -eq 124 ]]; then
            log_error "Test '$test_name' timed out after ${timeout}s"
            FAILED_SUITES+=("$test_name")
            SUITE_RESULTS+=("TIMEOUT: $test_name (${timeout}s)")
            
            echo "Result: TIMEOUT" >> "$FULL_REPORT"
            echo "Duration: ${timeout}s (timeout)" >> "$FULL_REPORT"
            echo "Test exceeded maximum allowed time of ${timeout} seconds" >> "$test_report"
            
            # Check if this is a critical test and abort if it failed
            if is_critical_test "$test_name"; then
                echo -e "\n${RED}CRITICAL TEST TIMEOUT DETECTED${NC}"
                echo -e "${RED}Test '$test_name' is part of a critical suite and has timed out.${NC}"
                echo -e "${RED}Aborting entire test run as critical prerequisites are not met.${NC}"
                echo
                echo "Critical test timeout: $test_name" >> "$FULL_REPORT"
                echo "Test run aborted due to critical timeout" >> "$FULL_REPORT"
                
                # Generate final report before exiting
                generate_final_report
                print_info "Partial test report available at: $FULL_REPORT"
                exit 1
            fi
            ...
            # Check if this is a critical test and abort if it failed
            if is_critical_test "$test_name"; then
                echo -e "\n${RED}CRITICAL TEST FAILURE DETECTED${NC}"
                echo -e "${RED}Test '$test_name' is part of a critical suite and has failed.${NC}"
                echo -e "${RED}Aborting entire test run as critical prerequisites are not met.${NC}"
                echo
                echo "Critical test failure: $test_name" >> "$FULL_REPORT"
                echo "Test run aborted due to critical failure" >> "$FULL_REPORT"
        ...
        # Non-verbose mode: capture output silently with timeout
        if timeout "$timeout" "$script" > "$test_report" 2>&1; then
            end_time=$(date +%s)
            duration=$((end_time - start_time))
...
Options:
  ...
  --verbose, -v       Show test output in real-time (verbose mode)
...
Test Timeouts:
  Tests have a maximum runtime (default: 60 seconds). Tests can specify custom timeouts
  using 'TEST_TIMEOUT: <seconds>' in their script header (after TEST_DESCRIPTION).
  Current timeout configuration:
    - Environment/Connectivity: 60-120s
...
Critical Suites:
  ...
  suite fails, the entire test run is immediately aborted since these are prerequisite
  validations required for other tests to function properly.
...
Examples:
  ...
  $0 --verbose                 Run with real-time output (helpful for debugging)
  $0 --verbose 10-test_environment  Run specific test with verbose output
...
main() {
    ...
    if [[ "$VERBOSE_MODE" == "true" ]]; then
        echo -e "Verbose mode: ${GREEN}ENABLED${NC} (real-time output)"
    fi
    echo
